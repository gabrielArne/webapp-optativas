...
def propuesta_detail(
    ...
    if not propuesta:
        flash(request, "Proyecto inexistente.", "danger")
        return RedirectResponse("/propuestas", status_code=303)
    return templates.TemplateResponse(
...
def download_propuesta_documentacion(
    ...
    if not propuesta or not propuesta.documentacion_ruta_archivo:
        flash(request, "El proyecto no tiene documentacion complementaria.", "danger")
        return RedirectResponse("/propuestas", status_code=303)

    file_path = Path(propuesta.documentacion_ruta_archivo)
    if not file_path.exists():
        flash(request, "No se encontro el archivo del proyecto.", "danger")
...
def apply_propuesta(
    ...
    if not propuesta or propuesta.estado != EstadoPropuesta.ABIERTA.value:
        flash(request, "El proyecto no esta disponible.", "danger")
        return RedirectResponse("/propuestas", status_code=303)
    ...
    db.add(postulacion)
    try:
        notify(db, propuesta.docente_id, f"Nuevo alumno postulado al proyecto: {propuesta.titulo}")
        db.commit()
        flash(request, "Postulacion enviada.")
    except IntegrityError:
        db.rollback()
        flash(request, "Ya te postulaste a ese proyecto.", "warning")
    return RedirectResponse(f"/propuestas/{propuesta.id}", status_code=303)
...
def update_postulacion(
    ...
    notify(db, postulacion.alumno_id, f"Tu postulacion al proyecto '{postulacion.propuesta.titulo}' cambio a {estado}.")
    db.commit()
...
def finish_propuesta(
    ...
    if not propuesta or (current_user.rol == RolUsuario.DOCENTE.value and propuesta.docente_id != current_user.id):
        flash(request, "No tenes acceso a ese proyecto.", "danger")
        return RedirectResponse("/propuestas", status_code=303)
    propuesta.estado = EstadoPropuesta.FINALIZADA.value
    db.commit()
    flash(request, "Proyecto finalizado.")
    return RedirectResponse(f"/propuestas/{propuesta.id}", status_code=303)
